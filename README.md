...
**The result is one demo executable both threads, maintains the Ring Buffer, and displays live stats.**
...
The organization of a C++ project is like this:
...
To put in simple words: headers create templates/classes that are commonly reused through multiple functions. Source files implement the actual functions. Each file is compiled indepedently, which is why the header needs to act as a place holder for the functions, or else the compiler does not know how the input/output of the functions. The linker joins everything, and the entry point isthe main file can call the functions in the source files. Also you can link multiple libraries, that's why you need namespace conventions to prevent collisions of similar classes.

...
To put in simple words: headers create templates/classes that are commonly reused through multiple functions. Source files implement the actual functions. Each file is compiled indepedently, which is why the header needs to act as a place holder for the functions, or else the compiler does not know how the input/output of the functions. The linker joins everything, and the entry point isthe main file can call the functions in the source files.
